...
exports.run = (client, message, args, config, blacklist) => {
	...
	var testDisable = ""
        ...
        if (testDisable == "both" || testDisable == "local") return message.channel.send("IT APPEARS THAT THE SERVER HAS DISABLED THE LOCAL SCORING SYSTEM. THEREFORE, I CAN'T SHOW YOUR SCORE. SORRY ABOUT THAT.");
        ...
        sql.get("SELECT * FROM '" + message.guild.id + "_score' WHERE userId = " + getScoreFrom).then(row => {
            if (!row) return message.channel.send("SCORE DATA DOESN'T SEEM TO EXIST... YET...");
            let curLevel = Math.floor(0.05 * Math.sqrt(row.points / 2));
            var p1 = row.level + 1
            var p2 = p1 / 0.2
