...
function playerRankGrabber(message, leaderboardConstruction, listModifier) {
    var playerRnk = []
    var position = 1
    sql.each("SELECT * FROM 'scores' ORDER BY points DESC", (err, row) => {
...
exports.run = (client, message, args, config, blacklist) => {
    ...
    sql.get("SELECT * FROM 'levelDisabling' WHERE serverID = ?", [message.guild.id]).then(row => {
        ...
        if (testDisable == "both" || testDisable == "global") return message.channel.send("IT APPEARS THAT THE SERVER HAS DISABLED THIS SERVER'S INVOLVEMENT IN GLOBAL LEADERBOARDS. THEREFORE, I CAN'T SHOW YOU THE LEADERBOARD. SORRY ABOUT THAT.");

        var listModifier = 10
        if (parseInt(args[0])) var listModifier = parseInt(args[0]) * 10
        var leaderboardConstruction = []
        var position = 0
        sql.each("SELECT * FROM 'scores' ORDER BY points DESC LIMIT ?", [listModifier], (err, row) => {
            position++
            if (!row) {}
            else {
